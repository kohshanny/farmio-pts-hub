    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-2xl md:text-3xl text-primary">Team overview</h1>
          <p className="text-sm text-ink-soft mt-1">All part-time agents, at a glance.</p>
        </div>
        <Link
          href="/entry"
          className="shrink-0 bg-primary text-white rounded-lg px-3 py-2 md:px-4 md:py-2.5 text-sm font-medium hover:bg-primary-soft transition-colors"
        >
          + Log order
        </Link>
      </div>

      {/* Stat cards — stack on mobile, 3-col on md+ */}
            ...
            <tbody>
              {leaderboard.map(({ agent, revenue, commissionPending, commissionPaid, orderCount, target }) => (
                <tr key={agent.id} className="border-b border-border last:border-0 hover:bg-bg/60">
                  ...
                  <td className="px-4 py-3 text-ink-soft whitespace-nowrap">{orderCount}</td>
                  <td className="px-4 py-3 whitespace-nowrap">{formatSGD(revenue)}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    {target > 0 ? (
                  ...
                  <td className="px-4 py-3 text-fresh whitespace-nowrap">{formatSGD(commissionPaid)}</td>
                  <td className="px-4 py-3 text-gold whitespace-nowrap">{formatSGD(commissionPending)}</td>
                </tr>
              ))}
            </tbody>
          </table>
