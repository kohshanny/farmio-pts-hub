...
export async function InternalOverview() {
  ...
  const newRevenue = allOrders.filter((o) => o.order_type === 'New').reduce((sum, o) => sum + o.revenue_sgd, 0);
  ...
  const leaderboard = allAgents
    .map((agent) => {
      ...
      const commissionOwed = agentOrders
        .filter((o) => o.commission_status === 'Pending')
        .reduce((sum, o) => sum + o.commission_amount, 0);
      ...
      return { agent, revenue, commissionOwed, commissionPaid, orderCount: agentOrders.length, target };
    })
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={`${formatSGD(newRevenue)} / ${formatSGD(recurringRevenue)}`}
          sublabel="Revenue split"
          accent="fresh"
        />
        <StatCard
          label="Active agents"
        ...
        <StatCard
          ...
          value={formatSGD(pendingCommissions)}
          sublabel="Across all agents, pending"
          accent="gold"
          icon={<Wallet size={16} />}
        />
      </div>
      ...
      <div>
        ...
        <div className="bg-surface border border-border rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {leaderboard.map(({ agent, revenue, commissionOwed, commissionPaid, orderCount, target }) => (
                <tr key={agent.id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-4 py-3 text-gold">{formatSGD(commissionOwed)}</td>
                </tr>
              ))}
            </tbody>
          </table>
